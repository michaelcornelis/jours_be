...
// Load comments
const commentsButton = document.querySelector(".js-content__comments-button");

if (commentsButton) {
    commentsButton.addEventListener("click", () => {   
        commentsButton.classList.toggle("is-hidden");      
        const container = document.querySelector(".js-content__comments-inner");   
        if (container) {
...
(function() {
	...
	const hideNewsletterPopup = () => {
		...
		window.removeEventListener('scroll', showNewsletterOnScroll); // Usuwamy event listener
		clearTimeout(showNewsletterTimeout); // Usuwamy timeout
	};

	let showNewsletterTimeout;
	...
	// Newsletter display
	const isSubscribed = localStorage.getItem('newsletter-subscribe');
	...
	if (isSubscribed !== 'true') {
		...
		if (showAfterTime) {
			showNewsletterTimeout = setTimeout(() => {
				newsletter.classList.add('is-visible');
			}, showAfterTime);
		}
	}
...
// Reading scroll bar
const contentBarProgress = document.getElementById("js-contentbar__progress");

if (contentBarProgress) { 
    document.addEventListener("scroll", () => {
        ...
        contentBarProgress.style.setProperty("--scroll", scrollPercent);
    }, { passive: true });
}
...
const contentBar = document.getElementById("js-content__bar");
...
if (contentBar) {
    window.addEventListener('scroll', () => {
        ...
        // Scrolling down
        if (currentScrollPosition < previousScrollPosition && previousScrollPosition > 300) {
            contentBar.classList.remove("is-hidden");
            contentBar.classList.add("is-visible");
        }

        if (previousScrollPosition < 300) {
            contentBar.classList.remove("is-visible");
        }
